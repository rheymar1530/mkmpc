	...
	<div style="border: 1px solid;padding-top:0.6;">
		<div class="row" style="border-bottom: 1px solid;">
			<div class="columnLeft">
				<h2 class="my-0 px-2" style="margin-top:0.5cm !important">{{$type}} DISBURSEMENT VOUCHER</h2>
			</div>
			...
			<div class="row" style="padding:0.6rem;padding-bottom: 0;padding-left: 0;padding-right: 0;">
				<table class="ftsize" width="100%">
					...
					<tr>
						<td class="td-p" colspan="3">Name: <span class="fnormal">{{$cdv_details->payee}}</span></td>
						<td class="td-p">Date: <span class="fnormal">{{$cdv_details->date}}</span></td>
					</tr>
					<tr>
						<td class="td-p" colspan="3">Address: <span class="fnormal">{{$cdv_details->address}}</span></td>
						<td class="td-p">CDV No.: <span class="fnormal">{{$cdv_details->id_cash_disbursement}}</span></td>
					</tr>
					<tr class="">
						<td class="td-p b" style="border-left: none !important;">Check No.  <span class="fnormal">{{$cdv_details->check_no}}</span></td>
						<td class="td-p b">Check Date:  <span class="fnormal">{{$cdv_details->check_date}}</span></td>
						<td class="td-p b">Amount: <span class="fnormal">{{number_format($cdv_details->total,2)}}</span></td>
						<td class="td-p b">Branch: <span class="fnormal">{{$cdv_details->branch_name}}</span></td>
					</tr>
				</table>	
			</div>
			<div class="row" style="padding:0.6rem">
				<p class="my-0 ftsize"><b>Particulars: </b><span class="fnormal">{{$cdv_details->description}}</span></p>
			</div>
			<div class="row" style="padding:0.6rem;padding-bottom: 0;padding-left: 0;padding-right: 0;">
				<table class="ftsize">
					...
					<tbody>
						...
						?>
						@foreach($cdv_items as $cdv)
						<tr>
							<td class="table_entry_font td-p br">{{$cdv->account_code}}</td>
							<td class="table_entry_font td-p br">{{$cdv->description}}</td>

							<td class="table_entry_font class_amount td-p br">{{ ($cdv->debit > 0)?number_format($cdv->debit,2):''}}</td>
							<td class="table_entry_font class_amount td-p br">{{ ($cdv->credit > 0)?number_format($cdv->credit,2):''}}</td>
							<td class="table_entry_font td-p" style="font-size: 10pt !important">{{$cdv->details}}</td>
							<?php
			...
			<div class="row" style="padding-bottom: 0;padding-left: 0;padding-right: 0;">
				<table class="ftsize">
					...
					<tr>
						<td class="class_amount br b-top b-bottom table_entry_font">{{number_format($loanDetails->principal,2)}}</td>
						<td class="class_amount br b-top b-bottom table_entry_font">{{number_format($loanDetails->interest,2)}}</td>
						<td class="class_amount br b-top b-bottom table_entry_font">{{number_format($loanDetails->total_due,2)}}</td>
						<td class="br b-top b-bottom table_entry_font" style="padding-left: 3mm">@if($loanDetails->startDate == $loanDetails->endDate) {{$loanDetails->startDate}} @else {{$loanDetails->startDate}} - {{$loanDetails->endDate}} @endif</td>
					</tr>
				</table>
			</div>
					@endif			
				...
				<table class="" id="tbl_m">
					...
					<tr>
						<td class="td-p br ftsize" style="padding-bottom: 0.5cm;">Prepared by: <br> <br>
							<span class="fnormal" style="padding-top:1.5cm !important">&nbsp;{{$prepared_by}}</span>
						</td>
						...
						<td colspan="3"  style="vertical-align: top !important;"><span class="fnormal">Received from <u><b class="fbold">{{env('APP_NAME')}}</b></u> the amount of {{$amt_worded}} Only.</span></td>
					</tr>
					<tr>
						<td class="td-p br b-top ftsize" style="padding-bottom: 0.5cm;">Approved by: <br> <br>
							<span class="fnormal" style="padding-top:1.5cm !important">&nbsp;{{config('variables.v_approved_by')}}</span>
						</td>
						<td class="td-p br b-top ftsize" style="padding-bottom: 0.5cm;">Released by: <br> <br>
							<span class="fnormal" style="padding-top:1.5cm !important">&nbsp;{{config('variables.v_released_by')}}</span>
						</td>
						<td colspan="3" class="ftsize" style="vertical-align:bottom;">
							<div class="row">
								...
								<div class="columnRec" style="text-align:center;border-bottom: 1px solid;">
									...
									<span style="text-align:center;width: 100%">{{$cdv_details->payee}}</span>&nbsp;&nbsp;
									<!-- <br>
