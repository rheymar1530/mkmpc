								...
								<div class="table-responsive" style="max-height: calc(100vh - 200px);overflow-y: auto;margin-top: 5px !important;overflow-x: auto">
									<table class="table table-bordered table-stripped table-head-fixed tbl-inputs" style="white-space: nowrap;">
										...
										<tbody id="loan_dues_body">
											...
											?>
											@foreach($loan_dues as  $c=>$loan)
											@foreach($loan as $k=>$dues)
											<?php 
											$class= ($c >0 && $k == 0)?"border-top":"";
											?>

											@if($dues['paid_amount'] > 0)
											<tr class="{{$class}}">
												<td class="tbl-inputs-text">
													{{$dues['loan_service']}}
													@if($dues['type'] == "previous")
													<small>({{$dues['service_remarks']}})</small>
													@endif
												</td>
												@if(!$is_cancelled)
												<td class="tbl-inputs-text class_amount">{{number_format($dues['amount'],2)}}</td>
												@endif
												<td class="tbl-inputs-text class_amount" style="padding-right:12px !important">{{number_format($dues['paid_amount'],2)}}</td>
											</tr>
											<?php
											$total_loan_paid += $dues['paid_amount'];
											$total_loan_due += $dues['amount'];
											?>
											@endif
										...
										<tfoot>
											<tr>
												...
												@if(!$is_cancelled)
												<td class="class_amount text_bold tbl-inputs-text" id="txt_total_loan_due">{{number_format($total_loan_due,2)}}</td>
												@endif
												<td class="class_amount text_bold tbl-inputs-text" id="txt_col_total_payment" style="padding-right:12px !important">{{number_format($total_loan_paid,2)}}</td>
											</tr>
										</tfoot>
									</table>    
								</div>  
