            ...
            <table id="tbl_member_list" class="table table-hover table-striped" style="white-space: nowrap;margin-top: 10px;">
                ...
                <tbody id="list_body">
                	@foreach($lists as $list)
                		<tr class="member_row" data-code="{{$list->member_code}}">
...
$(function() {
    $.contextMenu({
        ...
        callback: function(key, options) {
			...
			if(key == "view"){
				window.location = '/member/view/'+member_code+'?href='+'{{urlencode(url()->full())}}';
			}
...
function redirect_add(){
	window.location = '/member/create'+'?href='+'{{urlencode(url()->full())}}';
}
