	...
	<table class="table table-bordered table-stripped table-head-fixed table-hover tbl_accounts tbl_gl center" style="margin-top:20px;width:80% !important;">
		<thead>
			<tr style="text-align:center;" class="table_header_dblue_cust">
				<th class="table_header_dblue_cust" colspan="4" style="text-align:center;font-size: 17px">{{strtoupper($type)}}</th>
			</tr>
			...
			@foreach($transaction as $account=>$row)
			@if(count($row) == 1)
			@foreach($row as $item)
			<tr>
				<td colspan="2">{{$item->account}}</td>
				<!-- <td></td> -->
				<td class="class_amount">{{check_zero($item->debit)}}</td>
				<td class="class_amount pad_left">{{check_zero($item->credit)}}</td>
			</tr>
			<?php
			...
			<tr>
				<td colspan="2">&nbsp;&nbsp;</td>
				<td class="class_amount ovrline">{{check_zero($total_loan_deb)}}</td>
				<td class="class_amount pad_left ovrline" >{{check_zero($total_loan_cred)}}</td>
			</tr>
			...
			<tr class="row_total">
				...
				<th class="class_amount f-total ovrline" style="font-weight: bold;font-size: 12px;">{{check_zero2($total_debit)}}</th>
				<th class="class_amount f-total pad_left ovrline" style="font-weight: bold;font-size: 12px;">{{check_zero2($total_credit)}}</th>
			</tr>
	</table>  
</div>
@endforeach