...
<table class="table table-bordered table-stripped table-head-fixed table-hover tbl_accounts tbl_gl" style="margin-top:20px">
	...
	<thead>
		<tr style="text-align:center;" class="table_header_dblue_cust">
			<th class="table_header_dblue_cust" colspan="{{($type=='Cash Receipt Voucher')?9:8}}" style="text-align:center;font-size: 17px">{{strtoupper($type)}}</th>
		</tr>
		...
		@if(count($transaction) > 0)
			@foreach($transaction as $ref=>$trans_ref)
				...
				<tr class="row_trans">
					...
					<td>{{$items->account_code}} || {{$items->account_name}}</td>
					<?php
					...
					<td style="padding-left: 10px">{{$items->details}}</td>
				...
				<tr class="row_trans row_total" style="border-bottom:1px solid;">
					<td style="font-weight:bold;text-align:right;" colspan="{{($type=='Cash Receipt Voucher')?5:4}}"></td>
					<td style="font-weight:bold;text-align:left;">TOTAL</td>
					<td style="font-weight:bold" class="class_amount">{{check_amt($ac_debit)}}{{($items->status==10?'*':'')}}</td>
					<td style="font-weight:bold" class="class_amount">{{check_amt($ac_credit)}}{{($items->status==10?'*':'')}}</td>
					<td></td>
				</tr>
			@endforeach
				<tr class="row_total" style="border-bottom:1px solid;">
					<td style="font-weight:bold;font-size: 12px;" colspan="{{($type=='Cash Receipt Voucher')?6:5}}">{{$type}} Grand Total</td>
					<td style="font-weight:bold;font-size: 12px" class="class_amount">{{check_amt($debit_g_total)}}</td>
					<td style="font-weight:bold;font-size: 12px" class="class_amount">{{check_amt($credit_g_total)}}</td>
					<td></td>
				</tr>
		@endif
