	...
	<div class="row">
       <div class="col-md-12">
            <table id="tbl_jv" class="table table-hover table-striped" style="white-space: nowrap;margin-top: 10px;">
                ...
                <tbody id="list_body">
                	@foreach($jv_list as $list)
                		<tr class="jv_row" data-code="{{$list->id_journal_voucher}}">
                			...
                			<td class="class_amount">{{number_format($list->total_amount,2)}}</td>
...
@include('journal_voucher.filter_option_modal')
@endsection
@push('scripts')
...
$(function() {
    $.contextMenu({
        ...
        callback: function(key, options) {
			...
			if(key == "view"){
				window.location = '/journal_voucher/view/'+id_journal_voucher +'?href='+'{{urlencode(url()->full())}}';
			}else if(key == "print"){
...
function redirect_add(){
	window.location = '/journal_voucher/create'+'?href='+'{{urlencode(url()->full())}}';
}
