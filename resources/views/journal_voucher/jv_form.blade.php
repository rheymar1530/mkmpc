...
?>
	...
	<div class="container-fluid main_form section_body" id="jv_main_div" style="margin-top: -20px;" >
		...
		<a class="btn bg-gradient-secondary btn-sm" href="{{$back_link}}" style="margin-bottom:10px"><i class="fas fa-chevron-circle-left"></i>&nbsp;&nbsp;Back to Journal Voucher List</a>
		...
		<div class="card" id="repayment_main_div">
			...
			<div class="card-body col-md-12">
				<h3 class="text-center head_lbl">Journal Voucher 
					@if($opcode ==1)
						(ID# {{$jv_details->id_journal_voucher}})
						@if($jv_details->status == 10)
							...
							<div class="row">
								<div class="col-md-6">
									<div class="form-group row p-0" >
										...
										<div class="col-sm-9">
											<select class="form-control form_input p-0 jv_parent" id="sel_jv_type" key="jv_type">
												@foreach($jv_type as $val=>$desc)
													<?php
														$selected = (isset($jv_details->jv_type)?$jv_details->jv_type:1);
													?>
												<option value="{{$val}}" <?php echo ($selected == $val)?'selected':'';?> >{{$desc}}</option>
												@endforeach
											</select>
										</div>
									</div>
									...
									<div class="form-group row p-0">
										...
										<div class="col-sm-9">
											<input type="text" name="" class="form-control jv_parent" id="txt_address" key="address" value="{{$jv_details->address ?? ''}}">
										</div>
									</div>
								</div>
								<div class="col-md-6">
									<div class="form-group row p-0" >
										...
										<div class="col-sm-9">
											<select class="form-control form_input p-0 jv_parent" id="sel_branch" key="id_branch" required>
												...
												<option value="{{$branch->id_branch}}" <?php echo (isset($jv_details->id_branch)  && $jv_details->id_branch == $branch->id_branch)?'selected':''; ?> >{{$branch->branch_name}}</option>
												@endforeach
											</select>
										</div> 
									</div>
									<div class="form-group row p-0">
										...
										<div class="col-sm-9">
											<input type="text" name="" class="form-control jv_parent" id="txt_reference" key="reference" value="{{$jv_details->reference ?? ''}}">
										</div>
									</div>
									<div class="form-row">
						...
						<div class="col-md-12">
							<!-- <div class="card" style="margin-top:10px;" id="div_jv"> -->
									...
									<div class="table-responsive" style="max-height: calc(100vh - 300px);overflow-y: auto;margin-top: 5px !important;overflow-x: auto">
										<table class="table table-bordered table-stripped table-head-fixed tbl_jv" style="white-space: nowrap;">
											...
											<tbody id="jv_body">
												...
												@else
													@foreach($chart_details  as $c=>$cd)
													<tr class="jv_row_display">
														<td>{{$c+1}}</td>
														<td>{{$cd->account_code}}</td>
														<td>{{$cd->description}}</td>
														<td class="class_amount" style="padding-right:10px !important">{{($cd->debit==0)?'':number_format($cd->debit,2)}}</td>
														<td class="class_amount" style="padding-right:10px !important">{{($cd->credit==0)?'':number_format($cd->credit,2)}}</td>
														<td>{{$cd->details}}</td>
														<td></td>
													</tr>
													<?php
								...
								<!-- </div> -->
												...
												@if($opcode == 1)
							<div class="mt-3 ml-2">
								<span class="text-muted spn_details"><b>Date Created:</b> {{$jv_details->date_created}}</span><br>
								@if($jv_details->status == 10)
								<span class="text-muted spn_jv_details"><b>Date Cancelled:</b> {{$jv_details->date_cancelled}}</span><br>
								<span class="text-muted spn_jv_details"><b>Cancellation Reason:</b> {{$jv_details->cancellation_reason}}</span>
								@endif
							</div>
					@endif
						<!-- </div> -->
			...
			<div class="card-footer">
				@if($allow_post)
					...
					@if($opcode == 1 && $credential->is_cancel)
				...
				@if($opcode == 1)
				<button type="button" class="btn bg-gradient-danger2 float-right" style="margin-right:10px" onclick="print_page('/journal_voucher/print/{{$jv_details->id_journal_voucher}}')"><i class="fas fa-print" ></i>&nbsp;Print Journal Voucher</button>
				@endif
...
@if($opcode == 1 && $credential->is_cancel)
@include('journal_voucher.cancellation_modal')
@endif
...
<script type="text/javascript">
	function redirect_scheduler(){
		@if($jv_details->id_scheduler > 0)
			window.open('/scheduler/view/'+{{$jv_details->id_scheduler}},'_blank');
		@else
			var p = {
				type : 1,
				reference : {{$jv_details->id_journal_voucher ?? 0}}
			};
		...
		$.ajax({
			...
			data         :          {'jv_parent'  : jv_parent,
									             'chart_entry' : chart_entry,
									           	 'opcode'   :   OPCODE,
									           	  'id_journal_voucher' : {{$jv_details->id_journal_voucher ?? 0}}},
			beforeSend   :          function(){
									show_loader();
			},
			success      :          function(response){
