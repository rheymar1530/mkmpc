		...
		<div class="col-md-12">
			<table id="tbl_prime" class="table table-hover table-striped" style="white-space: nowrap;margin-top: 10px;">
				...
				<tbody id="list_body">
					...
					<tr class="prime_request_row" data-code="{{$list->id_prime_withdrawal}}" cdv-id="{{$list->id_cash_disbursement}}">
						<td>{{$list->id_prime_withdrawal}}</td>
						<td>{{$list->member}}</td>
						<td>{{$list->reason}}</td>
...
$(function() {
	$.contextMenu({
		...
		callback: function(key, options) {
			var m = "clicked: " + key;
			var id_prime_withdrawal = $(this).attr('data-code');
			var id_cdv = $(this).attr('cdv-id');
			console.log({id_prime_withdrawal})
			if(key == "view"){
				window.location = '/prime_withdraw/view/'+id_prime_withdrawal +'?href='+'{{urlencode(url()->full())}}';
			}
			else if(key == "print"){
				print_page('/prime_withdraw/entry/'+id_prime_withdrawal);
