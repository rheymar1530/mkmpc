...
<div class="main_form" style="margin-top: -20px;">
		...
		<div class="card-body px-5">
			...
			<div style="max-height: calc(100vh - 140px);overflow-y: auto;">
				<table class="table table-bordered table-head-fixed tbl_pdc w-100">
					...
					<tbody>
						@foreach($ChangeList as $idRepayment=>$Changes)
						<?php 
						...
						<tr>
							@if($c == 0)
							<td class="centered-text text-center" rowspan="{{$rowCount}}">{{$Change->id_repayment}}</td>
							<td class="centered-text" rowspan="{{$rowCount}}">{{$Change->transaction_date}}</td>
							<td><?php echo $Change->change_for;?></td>
							@endif
							<td>{{$Change->check_date}}</td>
							<td>{{$Change->check_bank}}</td>
							<td class="text-right">{{number_format($Change->amount,2)}}</td>

							@if($c==0)
							<td class="text-right centered-text" rowspan="{{$rowCount}}">{{number_format($TotalCheckAmount,2)}}</td>
							<td class="centered-text" rowspan="{{$rowCount}}"><span class="badge badge-{{$Change->deposit_badge}} text-xs">{{$Change->deposit_status_description}}</span></td>
							<td class="text-right centered-text" rowspan="{{$rowCount}}">{{number_format($Change->change_balance,2)}}</td>
							<td class="centered-text text-center" rowspan="{{$rowCount}}">
								@if($Change->deposit_status == 2)
								<a class="btn bg-gradient-success2 btn-xs" onclick="ParseRepaymentDetails({{$Change->id_repayment}})">Create Change</a>
								@endif
							</td>
							@endif
						</tr>
	...
	<div class="card">
		<div class="card-body px-5" id="crd-repayment-details">
			...
			@if($opcode == 1)
			<div class="text-center mb-3">
				<h4 class="head_lbl">Change Payables <small>(ID# {{$change_details->id_change_payable}})</small></h4>
			</div>
			@endif
			<div class="row">
			...
			<div class="row">
				<div class="form-group col-md-3">
					...
					<input type="date" class="form-control form-control-border" id="txt_date" value="{{$change_details->date ?? MySession::current_date()}}">
				</div>
				<div class="form-group col-md-5">
					<label class="lbl_color mb-0">Remarks</label>
					<input class="form-control form-control-border" id="txt_remarks" value="{{$change_details->remarks ?? ''}}">
				</div>
			</div>
									...
									<tr class="row-other-income">
										...
										<td class="p-0">
											<select class="form-control sel-account p-0">
												...
												<option value="{{$ca->id_chart_account}}">{{$ca->description}}</option>
												@endforeach
											</select>
										</td>
...
@include('change-payable.member-modal')
@endsection
...
<script type="text/javascript">
	const OPCODE = {{$opcode}};
	const ID_CHANGE_PAYABLE = {{$change_details->id_change_payable ?? 0}};
