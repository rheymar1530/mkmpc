		...
		<div class="col-md-12">
			<table id="tbl-change" class="table-hover table-bordered table-striped" style="margin-top: 10px;">
				...
				<tbody id="list_body">
					...
					<tr class="change-row" data-code="{{$list->id_change_payable}}">
						<td class="text-center">{{$list->id_change_payable}}</td>
						<td>{{$list->date}}</td>
						<td><?php echo $list->change_for; ?></td>
						<td>{{$list->id_repayment}}</td>
						<td class="text-right">{{number_format($list->total_amount,2)}}</td>
						

						<td>
							
							<span class="badge bg-gradient-{{$list->status_badge}} text-xs">
...
$(function() {
	$.contextMenu({
		...
		callback: function(key, options) {
			...
			if(key == "view"){
				window.location = '/change-payable/view/'+id_change +'?href='+'{{urlencode(url()->full())}}';
			}else if(key == "edit"){
				window.location = '/change-payable/edit/'+id_change +'?href='+'{{urlencode(url()->full())}}';
			}else if(key == "print"){
...
function redirect_add(){
	window.location = '/change-payable/create'+'?href='+'{{urlencode(url()->full())}}';
}
