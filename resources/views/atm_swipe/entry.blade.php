...
<body>
	...
	<div class="text-center">
		...
		<p class="head_others mb-0">{{config('variables.coop_address')}}</p>
		<br>
		<h3 style="font-size:18px" class="head mb-0">CASH DISBURSEMENT VOUCHER</h3>
		<br>
	</div>
	<div class="row">
		<table width="100%" style="border-bottom: solid 2px;">
			<tr>
				<td class="font_head"  width="40%"><strong>Pay to:</strong>&nbsp;{{$cdv_details->payee}}</td>
				<td class="font_head"  width="30%" style="text-align:center;"><strong>Branch:</strong>&nbsp;{{$cdv_details->branch_name}}</td>
				<td class="font_head"  width="30%" style="text-align: right;margin-right: 1cm !important;"><strong>Date:</strong>&nbsp;{{$cdv_details->date}}</td>
			</tr>
			<tr>
				<td class="font_head"  width="40%" ><strong>Address:</strong>&nbsp;{{$cdv_details->address}}</td>
				<td class="font_head"  width="30%" style="text-align:center;">
					@if($cdv_details->paymode == 2)
					<strong>&nbsp;Check no. :</strong>&nbsp;{{$cdv_details->check_no}}
					@endif
				</td>
				<td class="font_head" width="30%" style="text-align: right;margin-right: 1.1cm !important;"><strong>Amount:</strong>&nbsp;{{number_format($cdv_details->total,2)}}</td>
				<!-- <td class="font_head" width="33%"><strong>CDV No. </strong>&nbsp;{{$cdv_details->id_cash_disbursement}}</td> -->
				<!-- <td class="font_head"  width="33%"></td> -->
			</tr>
		</table>
	</div>
	<div class="row">
		<table width="100%" style="border-top: solid 1px;margin-top: 2px;border-bottom: solid 2px;">
			...
			<tr>
				<td class="font_head">{{$cdv_details->description}}</td>
			</tr>
		</table>
	</div>
	<?php
	...
	<div class="row" >
		...
		<table style="width:100%;border:solid 1px;">
			...
			<tr>
				<td class="table_entry_font" width="2.5cm">{{$cdv->account_code}}</td>
				<td class="table_entry_font">{{$cdv->description}}</td>
				
				<td class="table_entry_font class_amount" width="2.5cm">{{ ($cdv->debit > 0)?number_format($cdv->debit,2):''}}</td>
				<td class="table_entry_font class_amount" width="2.5cm">{{ ($cdv->credit > 0)?number_format($cdv->credit,2):''}}</td>
				<td class="table_entry_font" style="font-size: 10px !important">{{$cdv->details}}</td>
				<?php
