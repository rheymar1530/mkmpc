		...
		<div class="row" style="border-bottom: 1px solid;">
			...
			<div class="row" style="padding:0.6rem;padding-bottom: 0;padding-left: 0;padding-right: 0;">
				<table class="ftsize" width="100%">
					...
					<tr>
						<td class="td-p" colspan="3">Received from: <span class="fnormal">{{$crv_details->payee}}</span></td>
						<td class="td-p">Date: <span class="fnormal">{{$crv_details->date}}</span></td>
					</tr>
					<tr>
						<td class="td-p" colspan="3">Address: <span class="fnormal">{{$crv_details->address}}</span></td>
						<td class="td-p">CRV No.: <span class="fnormal">{{$crv_details->id_cash_receipt_voucher}}</span></td>
					</tr>
					<tr class="">
						<td class="td-p b" style="border-left: none !important;">Payment Type:  <span class="fnormal">{{$crv_details->paymode}}</span></td>
						<td class="td-p b">OR/AR No:  <span class="fnormal">{{$crv_details->or_no}}</span></td>
						<td class="td-p b">Amount: <span class="fnormal">{{number_format($crv_details->total_amount,2)}}</span></td>
						<td class="td-p b">Branch: <span class="fnormal">{{$crv_details->branch_name}}</span></td>
					</tr>
				</table>	
			</div>
			<div class="row" style="padding:0.6rem">
				<p class="my-0 ftsize"><b>Particulars: </b><span class="fnormal">{{$crv_details->description}}</span></p>
			</div>
			<div class="row" style="padding:0.6rem;padding-bottom: 0;padding-left: 0;padding-right: 0;">
				<table class="ftsize">
					...
					<tbody>
						...
						?>
						@foreach($entries as $crv)
						<tr>
							<td class="table_entry_font td-p br">{{$crv->account_code}}</td>
							<td class="table_entry_font td-p br">{{$crv->description}}</td>

							<td class="table_entry_font class_amount td-p br">{{ ($crv->debit > 0)?number_format($crv->debit,2):''}}</td>
							<td class="table_entry_font class_amount td-p br">{{ ($crv->credit > 0)?number_format($crv->credit,2):''}}</td>
							<td class="table_entry_font td-p" style="font-size: 10pt !important">{{$crv->details}}</td>
							<?php
				...
				<table class="" id="tbl_m">
					...
					<tr>
						<td class="td-p br ftsize" style="padding-bottom: 0.5cm;">Prepared by: <br> <br>
							<span class="fnormal" style="padding-top:1.5cm !important">&nbsp;{{config('variables.treasurer')}}</span>
						</td>
