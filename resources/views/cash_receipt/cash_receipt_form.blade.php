...
<div class="container main_form section_body" style="margin-top:-15px">
	...
	<a class="btn bg-gradient-secondary btn-sm" href="{{$back_link}}" style="margin-bottom:10px"><i class="fas fa-chevron-circle-left"></i>&nbsp;&nbsp;Back to Cash Receipt list</a>
		...
		<div class="col-md-12">
			<div class="card">
				<form id="frm_submit_cash_receipt">
					<div class="card-body">
						<h3 class="lbl_color text-center">@if($opcode == 0)Create @endif Cash Receipt 
							@if($opcode == 1) <small>(ID# {{$details->id_cash_receipt}})</small>  &nbsp;             
						...
						<div class="row">
							<div class="col-md-12">
									...
									<div class="col-md-12 p-0">
										<div class="form-row">
											<div class="form-group col-md-6">
												...
												<input type="date" class="form-control cash_receipt" id="txt_received_date" key="date_received" value="{{$details->date_received ?? $current_date}}">
											</div>
										</div>
										<div class="form-row">
											...
											<div class="form-group col-md-8" id="form_member">
												...
												<select id="sel_id_member" class="form-control cash_receipt p-0" key="id_member" required>
													@if(isset($member_selected))
													<option value="{{$member_selected->tag_id}}">{{$member_selected->tag_value}}</option>
													@endif
												</select>
											</div>
										</div>
												...
												<div class="form-row mt-3">
													<div class="form-group col-md-3">
														<select id="sel_paymode" class="form-control cash_receipt p-0" key="id_paymode">
															@foreach($paymode as $pay)
															<option value="{{$pay->id_paymode}}">{{$pay->description}}</option>
															@endforeach
														</select>
													</div>
												</div>
													...
													<tr class="row_payments">
														<td>
															<select class="form-control frm-ben p-0 sel_id_payment" key="id_payment_type">
																...
																<option value="{{$p->id_payment_type}}" {{$selected_payment}}>{{$p->description}}</option>
																@endforeach
															</select>
														</td>
														<td><input type="text" name="" class="form-control frm-ben class_amount txt_pay_amount t_amount" value="{{number_format($pay->amount,2)}}" key="amount"></td>
								...
								<div style="padding-top: 10px !important;">
									<span class="text-muted spn_details"><b>Date Created:</b> {{$details->date_created}}</span><br>
									@if($details->status == 10)
									<span class="text-muted spn_details"><b>Date Cancelled:</b> {{$details->date_cancelled}}</span><br>
									<span class="text-muted spn_details"><b>Cancellation Reason:</b> {{$details->cancel_reason}}</span>
									@endif
								</div>
					...
					<div class="card-footer p-2">
						@if(($opcode == 0 && $credential->is_create) || ($opcode == 1 && $details->status != 10 && $credential->is_edit))
						<button class="btn bg-gradient-success2 float-right"><i class="far fa-save"></i>&nbsp;Save Cash Receipt</button>
						@endif
						@if($opcode == 1)
						<!-- <button type="button" class="btn bg-gradient-danger float-right" style="margin-right:10px" onclick="print_frame('{{$print_route}}')"><i class="fas fa-print" ></i>&nbsp;Print Cash Receipt</button> -->
						<div class="btn-group float-right" style="margin-right:10px">
							...
							<div class="dropdown-menu">
								@if($details->status != 10)
								<a class="dropdown-item" href="javascript:void(0)" onclick="print_frame('{{$print_route}}')">Print Cash Receipt</a>
								@endif
								<a class="dropdown-item" onclick="print_page('/cash_receipt_voucher/print/{{$details->id_cash_receipt_voucher}}')">Print CRV (CRV #{{$details->id_cash_receipt_voucher}})</a>
							</div>
						</div>

						@if($details->status != 10 && $credential->is_cancel)
						<button type="button" class="btn bg-gradient-warning2 float-right" style="margin-right:10px;color:white" onclick="show_cancel_modal('{{$details->id_cash_receipt}}')"><i class="fas fa-times" ></i>&nbsp;Cancel Cash Receipt</button>
						@endif
						@endif
						@if($opcode ==1)

						<!-- 	<button type="button" class="btn bg-gradient-danger float-right" style="margin-left:10px" onclick="print_page('/journal_voucher/print/{{$details->id_cash_receipt_voucher}}')"><i class="fas fa-print" ></i>&nbsp;Print Journal Voucher (JV# {{$details->id_cash_receipt_voucher}})</button> -->
						@endif
					</div>
				</form>
			</div>
...
@include('cash_receipt.cancel_modal')
@include('cash_receipt.or_print_modal')
...
<script type="text/javascript">
	...
	const opcode = {{$opcode}}; //0-add;1-edit
