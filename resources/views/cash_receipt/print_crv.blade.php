...
<body>
	<div class="text-center">
		...
		<br>
		<h3 style="font-size:18px" class="head mb-0">CASH RECEIPT VOUCHER</h3>
		<br>
	</div>
	...
	<div class="row">
		<table width="100%" style="border-bottom: solid 2px;">
			<tr>
				<td class="font_head"  width="40%"><strong>Pay to:</strong>&nbsp;{{$crv_details->payee}}</td>
				<td class="font_head"  width="30%" style="text-align:center;"><strong>Branch:</strong>&nbsp;{{$crv_details->branch_name}}</td>
				<td class="font_head"  width="30%" style="text-align: right;margin-right: 1cm !important;"><strong>Date:</strong>&nbsp;{{$crv_details->date}}</td>
			</tr>
			<tr>
				<td class="font_head"  width="33%" colspan="2"><strong>Address:</strong>&nbsp;{{$crv_details->address}}</td>
				<td class="font_head" width="50%" style="text-align: right;margin-right: 1cm !important;"><strong>Amount:</strong>&nbsp;{{number_format($crv_details->total_amount,2)}}</td>
	...
	<div class="row">
		<table width="100%" style="border-top: solid 1px;margin-top: 2px;border-bottom: solid 2px;">
			...
			<tr>
				<td class="font_head">{{$crv_details->description}}</td>
			</tr>
		</table>
	</div>
	<?php
	...
	<div class="row">
		...
		<table style="width:100%;border:solid 1px;">
			...
			<tr>
				<td class="table_entry_font" width="2.5cm">{{$crv->account_code}}</td>
				<td class="table_entry_font">{{$crv->description}}</td>
				
				<td class="table_entry_font class_amount" width="2.5cm">{{ ($crv->debit > 0)?number_format($crv->debit,2):''}}</td>
				<td class="table_entry_font class_amount" width="2.5cm">{{ ($crv->credit > 0)?number_format($crv->credit,2):''}}</td>
				<td class="table_entry_font" style="font-size:10px !important">{{$crv->details}}</td>
				<?php
