		...
		<div class="col-md-12">
			...
			<table id="tbl_cash_receipt" class="table table-hover table-striped" style="white-space: nowrap;margin-top: 10px;">
				...
				<tbody id="list_body">
					...
					<tr class="row_cash_receipt" data-key="{{$list->id_cash_receipt}}">
						<?php
							if($list->status == 10){
						...
						?>
						<td>{{$list->id_cash_receipt}}</td>
						<td>{{$list->date_received}}</td>
						<td>{{$list->payee}}</td>
						<td>{{$list->member_code}}</td>
						<td>{{$list->or_no}}</td>
						<td>{{$list->paymode}}</td>
						<td class="col_amount">{{$list->total_payment}}</td>
						<td>
							@if($list->status)
...
@include('cash_receipt.filter_option_modal')
...
$(document).ready(function(){
		$('#spn_total_payment').text("₱"+'{{$total_amount}}');
		$('#spn_cancelled_payment').text("₱"+'{{$cancelled_amount}}')
...
$(function() {
	$.contextMenu({
		...
		callback: function(key, options) {
			...
			if(key == "view"){
				window.location = '/cash_receipt/view/'+id_cash_receipt+'?href='+'{{urlencode(url()->full())}}';
			}else if(key == "print"){
...
function redirect_add(){
	window.location = '/cash_receipt/add'+'?href='+'{{urlencode(url()->full())}}';
}
