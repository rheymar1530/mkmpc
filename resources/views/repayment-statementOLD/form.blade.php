...
<div class="main_form" style="margin-top: -20px;">
				...
				<div class="card-body">
					...
					<div class="row">
						<div class="col-md-12">
							...
							?>
							@if(!$with_statement)
							<p class="text-muted font-italic">Due date as of {{date('m/d/Y',strtotime($date_due))}}</p>
							<div class="table">
								@include('repayment-statement.statement_table')
							</div>
							...
							<div class="alert alert-warning">
								<strong>
									@if($branch_details->type == 1)Barangay {{$branch_details->name}} @else {{$branch_details->name}} LGU @endif</strong> (Due date : {{date('m/d/Y',strtotime($date_due))}}) has already statement. <a href="/repayment-statement/view/{{$statement_details->id_repayment_statement}}">[Statement No. {{$statement_details->month_year}}-{{$statement_details->id_repayment_statement}}]</a>
									<br>   
								</div>
								@endif
							</div>
							@if(!$with_statement)
							<div class="col-md-12">
								<h6 class="lbl_color float-right">Statement Total: <span class="font-weight-bold ml-3" id="gtotal">{{number_format($GLOBALS['total'],2)}}</span></h6>
							</div>
							@endif
						</div>
					</div>
	...
	// $(document).on('keyup','.in-penalty',function(){
	// 	var parent_row = $(this).closest('tr');
	// 	let curDue = parseFloat(parent_row.attr('loan-due'));
	// 	var p = $(this).val();
	// 	let penalty = ($.isNumeric(p))?roundoff(p):decode_number_format(p);

	// 	let totalRow = (isNaN(penalty)?0:penalty)+curDue;

	// 	parent_row.find('input.in-total-payment').val(number_format(totalRow,2));
	// 	computeAll();
	// 	console.log({curDue,penalty});
	// });

	// const computeAll = ()=>{
	// 	let total_payment = 0;
	// 	$('.in-total-payment').each(function(){
	// 		let temp_total = decode_number_format($(this).val());
	// 		total_payment+=temp_total;
	// 	});
	// 	$('#gtotal').text(number_format(total_payment,2));
	// }
	...
	<script type="text/javascript">
		...
		// 	var tempData = {};
		// 	tempData['id_loan'] = $(this).attr('data-id');
		// 	tempData['penalty'] = decode_number_format($(this).find('.in-penalty').val());
		// 	DataLoan.push(tempData);
		// });
			let SelectedMembers = [];
