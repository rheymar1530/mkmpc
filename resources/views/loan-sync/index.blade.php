				...
				<div class="card-body">
						...
						<div class="row">
							<div class="col-md-9">
								<select class="form-control mb-3 p-0" name="q">
									...
									<option value="{{$mt->id_membership_type}}" <?php echo ($selected_type == $mt->id_membership_type)?'selected':''; ?> >{{$mt->description}}</option>
									@endforeach
					...
					<div class="mt-3" style="max-height: calc(100vh - 50px);overflow-y: auto;">
						<table class="table tbl_mem table-bordered table-head-fixed table-hover">
							...
							<tbody>
								@foreach($members as $brgy_lgu=>$mem)

								@if($brgy_lgu !== "")
								<tr>
									<td class="bg-gradient-success font-weight-bold">{{$brgy_lgu}}</td>
								</tr>
								@endif
								@foreach($mem as $m)
								<tr class="row-member " data-id="{{$m->id_member}}" title="Double click to add loan">
									<td class="td-member">{{$m->member}}@if($m->count > 0)<i class="fas fa-check-circle float-right spn-check"></i>@endif</td>
								</tr>
								@endforeach
								@endforeach
							</tbody>
					...
					<table class="table tbl-loans table-bordered table-head-fixed">
						...
						<tbody id="body-loan">
							<tr class="loan-row">
								...
								<td>
									<input type="date" class="form-control in-loan"  key="date_approved" value="{{MySession::current_date()}}">
								</td>
								<td>
