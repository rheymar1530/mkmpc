...
<!-- 											<tr>
											...
											<tr class="foot_loan">
												...
												<td class="class_amount text_bold tbl-inputs-text" id="txt_pay_principal" style="padding-right:12px !important">0.00</td>
												<td class="class_amount text_bold tbl-inputs-text" id="txt_pay_interest" style="padding-right:12px !important"></td>
												<td class="class_amount text_bold tbl-inputs-text" id="txt_pay_fees" style="padding-right:12px !important"></td>
			...
			<div class="card-footer">
				...
				@endif
				@if($opcode == 1)
				@if($repayment_transaction->transaction_type ==2 || $repayment_transaction->transaction_type ==3)
...
<script type="text/javascript">
	...
	function parseTotalLoanPayment(){
		...
		var total_principal=0,total_interest=0,total_fees=0;
		...
		$('tr.row_loan_dues').each(function(i){
			$(this).find('.txt_loan_payment_amount').each(function(){
				...
				console.log({val,total_principal,total_fees})
		...
		$('#txt_pay_total').text(number_format(total_loan_due_payment,2));
	...
	function display_loan_dues(loan_dues,total_loan_due_,is_load){
		...
		// if(!is_load){
		// 	paid_principal = (paid_principal ==0)?"":
		// }else{
		...
		$.each($loan_dues,function($x,$dues){
			
			out += '<tr>';
			var badge_class = ($x=="current")?"success":"danger";
			out += '	<th colspan="6" class="col_payment_type"><span class="badge badge_loan_type badge-'+badge_class+'">'+$x.toUpperCase()+'</span></th>';
			out += '</tr>';
			$.each($dues,function($c,$due){
				console.log({$due});


			var paid_principal = parseFloat($due.paid_principal);
			var paid_interest = parseFloat($due.paid_interest);
			var paid_fees = parseFloat($due.paid_fees);

			var total_due = paid_principal+paid_interest+paid_fees;

			if(is_load){
				paid_principal = number_format(paid_principal);
				paid_interest = number_format(paid_interest);
				paid_fees = number_format(paid_fees);
			}else{
			...
			out += "<tr class='row_loan_dues'>";

			out += '<td class="tbl-inputs-text">ID#<a href="/loan/application/approval/'+$due.loan_token+'" target="_blank">'+$due.id_loan+'</a> '+$due.loan_service_name+loan_status+'</td>'

			if($due.type == "current"){
				out += '<td><input type="text" class="form-control class_amount frm_loans" disabled value="'+number_format($due.principal_balance,2)+'"></td>'
			}else{
				out += '<td></td>';
			}
				
			out +=  '<td><input type="text" name="" required class="form-control frm_loans txt_loan_payment_amount txt_prin" key="amount" value="'+paid_principal+'" table-reference="'+$due.id_loan_table+'" loan-token="'+$due.loan_token+'" payment-type="'+$due.type+'" due="'+$due.principal+'" amount-type="principal" placeholder="('+number_format(parseFloat($due.principal),2)+')" title="('+number_format(parseFloat($due.principal),2)+')"></td>';
			var disabled = ($due.act_interest > 0)?"":"disabled";
			paid_interest = ($due.act_interest > 0)?paid_interest:"";
			var placeholder = ($due.act_interest > 0)?('('+number_format(parseFloat($due.interest),2)+')'):"";
			out +=  '<td><input type="text" name="" required class="form-control frm_loans txt_loan_payment_amount txt_in" key="amount" value="'+paid_interest+'" table-reference="'+$due.id_loan_table+'" loan-token="'+$due.loan_token+'" payment-type="'+$due.type+'" due="'+$due.interest+'" amount-type="interest" placeholder="'+placeholder+'" '+disabled+' title="'+placeholder+'"></td>';

			var disabled = ($due.act_fees > 0)?"":"disabled";
			paid_fees = ($due.act_fees > 0)?paid_fees:"";
			var placeholder = ($due.act_fees > 0)?('('+number_format(parseFloat($due.fees),2)+')'):"";
			out +=  '<td><input type="text" name="" required class="form-control frm_loans txt_loan_payment_amount txt_fees" key="amount" value="'+paid_fees+'" table-reference="'+$due.id_loan_table+'" loan-token="'+$due.loan_token+'" payment-type="'+$due.type+'" due="'+$due.fees+'" amount-type="fees" placeholder="'+placeholder+'" '+disabled+' title="'+placeholder+'"></td>';	

			out += '<td><input type="text" class="form-control class_amount frm_loans total_loan_paid" disabled value="'+number_format(total_due,2)+'"></td>'		
			out += "</tr>";

			if($due.type == "current"){
				$total_current += (parseFloat($due.principal)+parseFloat($due.interest)+parseFloat($due.fees));
				with_current = true;
			}else{
				$total_previous += (parseFloat($due.principal)+parseFloat($due.interest)+parseFloat($due.fees));
			}
			})
		})
		...
		if(with_current){
			$('#span_btn_half').html($btn_comp_half);
		}else{
			$('#btn_comp_half').remove();
		}
	...
	function compute_payment_row(obj){
	...
	$(document).on('dblclick','input.txt_loan_payment_amount',function(){
...
<script type="text/javascript">
	function post(){
		...
		$('.txt_loan_payment_amount').each(function(){
			...
			if(payment_type == "previous"){
				try{
					previous_payments[token][amount_type] = val;
				}catch(e){
				...
				try{
					current_payments[token][amount_type] = val;
				}catch(e){
    	...
    	var sp = ($('#sel_transaction_type').val() == 1 || $('#sel_transaction_type').val() == 3) ?0: decode_number_format($('#swiping_amount').val())
