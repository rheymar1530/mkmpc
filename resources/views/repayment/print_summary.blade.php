...
<body>
	...
	<div class="row">
		<table width="100%" style=";border-collapse: collapse;border-top: 1px solid black" class="tbl_repayment_summary">
			...
			<tbody>
				...
				<tr>
					...
					<!-- <td class="col_amount">{{$due}}</td> -->

					<td class="col_amount">{{$amt}}</td>
					<?php
						$total_due += $val->due;
		...
		<table style="border:none !important;width: 50%;margin-top: -0.3cm;font-size: 17px;" id="nb">
			@foreach($trans_summary as $key=>$desc)
				@if(isset($transaction_summary[$key]))
				<?php
				...
				<tr class="{{$class}}">
					<td>{{$desc}}</td>
					<td class="col_amount">{{$amt_text}}</td>
				</tr>
				@endif
			...
			<tr class="highlight2">
				...
				<td class="col_amount">{{number_format($total_paid,2)}}</td>
			</tr>	

			@if($trans != "Cash")
			<tr class="highlight2">
				<td>Total Amount Received</td>
				<td class="col_amount">{{number_format($total_payment_received,2)}}</td>
			</tr>
			...
			<tr class="highlight">
				<td>{{$change_key}}</td>
				<td class="col_amount">{{number_format($transaction_summary[$change_key][0]->amount,2)}}</td>
			</tr>
			@endif
		</table>
	</div>
