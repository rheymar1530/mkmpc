            ...
            <table id="tbl_payroll" class="table table-hover table-striped" style="white-space: nowrap;margin-top: 10px;">
                ...
                <tbody id="list_body">
                	@foreach($payroll as $list)
                		<tr class="payroll_row" data-code="{{$list->id_payroll}}" >
                			<td>{{$list->id_payroll}}</td>
                			<td>{{$list->date_released}}</td>
                			<td>{{$list->description}}</td>
                			<td>{{$list->period_start}}</td>
                			<td>{{$list->period_end}}</td>
                			<td>{{$list->no_days}}</td>
                			<td>{{$list->salary_mode}}</td>
                			<td>@if($list->status == 10)
...
$(function() {
    $.contextMenu({
        ...
        callback: function(key, options) {
			...
			if(key == "view"){
				window.location = '/payroll/view/'+id_payroll +'?href='+'{{urlencode(url()->full())}}';
			}else{
				$.ajax({
					type            :            'GET',
...
function redirect_add(){
	window.location = '/payroll/create'+'?href='+'{{urlencode(url()->full())}}';
}
