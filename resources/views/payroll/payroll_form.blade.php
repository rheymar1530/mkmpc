...
<div class="wrapper2">
	<div class="container-fluid main_form section_body" style="margin-top: -20px;" >
		...
		<a class="btn bg-gradient-secondary btn-sm" href="{{$back_link}}" style="margin-bottom:10px"><i class="fas fa-chevron-circle-left"></i>&nbsp;&nbsp;Back to Payroll List</a>
			...
			<form id="frm_payroll">
				<div class="card-body col-md-12">
					<h3 class="head_lbl text-center">Payroll
						@if($opcode == 1)
						<small>(ID# {{$payroll->id_payroll}})</small>
						@if($payroll->status == 10)
					...
					<div class="row">
						<div class="col-md-12">	
							<div class="form-group row p-0"  style="margin-top:15px">
								...
								<div class="col-md-3">
									<input type="date" class="form-control in_payroll" id="txt_date_released" key="date_released" value="{{$payroll->date_released ?? $current_date}}">
								</div>
							</div>
							<div class="form-group row p-0">
								...
								<div class="col-md-7">
									<select class="form-control in_payroll p-0 sel_employee"  key="disbursed_by" id="sel_disbursed_by">
										@if(isset($disbursed_by))
										<option value="{{$disbursed_by->tag_id}}">{{$disbursed_by->tag_value}}</option>
										@endif
							...
							<div class="form-group row p-0">
								...
								<div class="col-md-3">
									<select class="form-control in_payroll p-0" id="sel_id_payroll_mode" key="id_payroll_mode">
										@foreach($payroll_mode as $pm)
										<?php $selected_id_payroll_mode = $payroll->id_payroll_mode ?? 1 ;?>
										<option value="{{$pm->id_payroll_mode}}" <?php echo ($pm->id_payroll_mode == $selected_id_payroll_mode)?"selected":"";  ?> >{{$pm->description}}</option>
										@endforeach
									</select>
								</div>
							</div>										
							...
							<div class="form-group row p-0">
								...
								<div class="col-md-3">
									<input type="date" class="form-control in_payroll" id="sel_period_start" key="period_start" value="{{$payroll->period_start ?? $temp_period_start}}">
								</div>
								<div class="col-md-3">
									<input type="date" class="form-control in_payroll" id="sel_period_end" key="period_end" value="{{$payroll->period_end ?? $temp_period_end}}">
								</div>
							</div>
							...
							<div class="form-group row p-0" id="div_working_days">
								...
								<div class="col-md-2">
									<input type="number" class="form-control in_payroll" id="txt_no_days" key="no_days" value="{{$payroll->no_days ?? 0}}">
								</div>
							</div>
						</div>
						@if($allow_post)
						...
						<div class="col-md-12">
							...
							<!-- <div class="form-row">
								<div class="form-group col-md-12">
									...
									<textarea class="form-control in_payroll" rows="2" style="resize:none;" id="txt_remarks" key="remarks">{{$payroll->remarks ?? ''}}</textarea>
								</div>
							</div> -->
							@if($opcode == 1 && $payroll->status == 10)
							<p>Note: {{$payroll->cancellation_reason}}</p>
							@endif
						</div>
					</div>
				</div>
				<div class="card-footer">
					...
					<div class="btn-group float-right" style="margin-right:10px">
						...
						<div class="dropdown-menu">
							@if($allow_post)
							<a class="dropdown-item" href="javascript:void(0)" onclick="print_page('/payroll/print_summary/{{$payroll->id_payroll}}')" >Print Payroll Summary</a>
							<a class="dropdown-item" href="javascript:void(0)" onclick="print_page('/payroll/print_payroll_payslip/{{$payroll->id_payroll}}')" >Print Payslip</a>
							<div class="dropdown-divider"></div>
							<a class="dropdown-item" href="/payroll/excel_summary/{{$payroll->id_payroll}}" target="_blank">Export Payroll Summary (Excel)</a>
							<div class="dropdown-divider"></div>
							@endif
							<a class="dropdown-item" onclick="print_page('/cash_disbursement/print/{{$payroll->id_cash_disbursement}}')">Print Cash Disbursement (CDV #{{$payroll->id_cash_disbursement}})</a>
...
@include('payroll.status_modal')
@endif
@include('payroll.payroll_employee_modal')
@include('payroll.master_list')
