...
<table class="table table-bordered table-head-fixed" id="tbl_overdues" width="100%">
	...
	<thead>
		<tr class="nb-bottom">
			...
			<th class="table_header_dblue" colspan="3" <?php echo $style; ?>><b>DUES AS OF {{strtoupper($asOf)}}</b></th>
		...
		?>
			@foreach($loans as $c=>$loan)
				...
				<tr class="{{$class}}" tk="{{$exportMode==0?$loan->loan_token:0}}">
					...
					@endif
					<td>{{$loan->{$reference} }} &nbsp;@if($type < 3)<a <?php echo $ref;?> ><sup>[{{$loan->id_loan}}]</sup></a>@endif</td>
					...
					<td class="text-center">{{$loan->elapsed_month}}</td>


					<td class="class_amount">{{number_formats($loan->principal_due)}}</td>
					<td class="class_amount">{{number_formats($loan->interest_due)}}</td>
					<td class="class_amount">{{number_formats($loan->total_due)}}</td>
					<td class="class_amount">{{number_formats($loan->total_payment)}}</td>
			...
			@if(count($loans) > 1)
				...
				<tr class="bbottom">
					...
					<td></td>
					<td></td>
					<td class="class_amount" style="font-weight: bold">{{number_formats($Stotal['principal_due'])}}</td>
					<td class="class_amount" style="font-weight: bold">{{number_formats($Stotal['interest_due'])}}</td>
					<td class="class_amount" style="font-weight: bold">{{number_formats($Stotal['total_due'])}}</td>
					<td class="class_amount" style="font-weight: bold">{{number_formats($Stotal['total_payment'])}}</td>
	...
	<tr class="btop">
		...
		<td></td>
		<td></td>
		<td class="class_amount" style="font-weight: bold">{{number_formats($totals['principal_due'])}}</td>
		<td class="class_amount" style="font-weight: bold">{{number_formats($totals['interest_due'])}}</td>
		<td class="class_amount" style="font-weight: bold">{{number_formats($totals['total_due'])}}</td>
		<td class="class_amount" style="font-weight: bold">{{number_formats($totals['total_payment'])}}</td>
