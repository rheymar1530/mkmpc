...
<table class="table borderless table-head-fixed table-hover tbl_accounts tbl_gl mt-1">
	...
	<tbody>
		...
		@else
			@include('financial_statement.non-current')
		...
		<tr style="border-top: 1px solid;border-bottom: 2px solid;" class="row_total pad_head">
			...
			<td class="font-emp" style="font-weight:bold;font-size: 12px" colspan="3">TOTAL {{strtoupper($type)}} </td>
				@foreach($GLOBALS['totalType'][$type] as $k=>$tot)
		...
		<tr style="border-bottom: 2px solid;" class="pad_head">
			...
			<td class="class_amount col_border {{td_class($GLOBALS['totalType']['Liabilities'][$h] + $GLOBALS['totalType']['Equity'][$h])}}" style="font-weight: bold;"><u>{{check_negative($GLOBALS['totalType']['Liabilities'][$h] + $GLOBALS['totalType']['Equity'][$h])}}</u></td>
			@endif
			@endforeach
			@if($comparative_type ==1)
				@if($date_type == 1)
				<td class="class_amount col_border {{td_class($GLOBALS['totalType']['Liabilities']['LAST_MONTH'] + $GLOBALS['totalType']['Equity']['LAST_MONTH'])}}" style="font-weight: bold;"><u>{{check_negative($GLOBALS['totalType']['Liabilities']['LAST_MONTH'] + $GLOBALS['totalType']['Equity']['LAST_MONTH'])}}</u></td>
				@endif
				<td class="class_amount col_border {{td_class($GLOBALS['totalType']['Liabilities']['LAST_YEAR'] + $GLOBALS['totalType']['Equity']['LAST_YEAR'])}}" style="font-weight: bold;"><u>{{check_negative($GLOBALS['totalType']['Liabilities']['LAST_YEAR'] + $GLOBALS['totalType']['Equity']['LAST_YEAR'])}}</u></td>
			@endif
		</tr>
		@else
