...
<!-- 					<div class="col-md-3">
					...
					<div class="col-md-12">
						<h4 style="margin-bottom: 10px;" class="head_lbl text-center">{{$description}}</h4>
						<br>
<!-- 
						...
						<div class="form-group row" >
							...
							<div class="col-md-3">
								<select class="form-control p-0" id="sel_cash_bank">
									@foreach($cash_bank as $val=>$tl)
									<option value="{{$val}}" <?php echo($val == $sel_cash_bank)?"selected":""; ?> >{{$tl}}</option>
									@endforeach
								</select>
							</div>
						</div>
...
<script type="text/javascript">
	function generate_transaction_summary(type,export_type=1){
		...
		if(type == 1){
			window.location = '/journal/report/{{$fil_type}}?'+queryString;
		}else{
			window.open('/journal/report/{{$fil_type}}/export?'+queryString,'_blank');
		}
