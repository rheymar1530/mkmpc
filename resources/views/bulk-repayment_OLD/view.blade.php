...
<div class="container-fluid">
	...
	@if($details->status == 0)
	<a class="btn bg-gradient-primary2 btn-sm round_button float-right" href="/bulk-repayment/edit/{{$details->id_repayment}}?href={{$back_link}}" style="margin-bottom:10px"><i class="fas fa-edit"></i>&nbsp;&nbsp;Edit Loan Payment</a>
	@endif
	<div class="card">
		<div class="card-body">
					...
					<div class="row">
						...
						<div class="col-lg-6 col-12">
							<div class="d-flex flex-column">
								...
								<span class="text-sm  font-weight-bold lbl_color">Check Type: <span class="ms-sm-2 font-weight-normal ml-2">{{$details->paymode}}</span></span>
			...
			<div class="table-responsive mt-2">
				<table class="table table-bordered table-head-fixed tbl_pdc w-100" id="tbl_pdc">
					<thead>
						...
						<tbody class="borders mbody" data-id="{{$id_member}}">
							...
							<tr>
								...
								<td class="text-right">{{number_format($l->penalty,2)}}</td>
								<td class="text-right">{{$rebates}}</td>
								@if($c == 0)
								<td class="text-right vcenter" rowspan="{{count($loan)}}">{{number_format($cbu_amount,2)}}</td>
								<td class="text-right font-weight-bold mtotal" rowspan="{{count($loan)}}">0</td>
								<td class="text-center" rowspan="{{count($loan)}}"><a class="btn btn-xs bg-gradient-danger" onclick="print_page('/cash_receipt_voucher/print/{{$l->id_cash_receipt_voucher}}')"><i class="fa fa-print"></i>&nbsp;Print CRV</a></td>
								@endif
					...
					<footer>
						<tr>
							<th colspan="6" class="footer_fix text-left font-weight-normal" style="text-align:center;background: #808080 !important;color: white;"></th>
