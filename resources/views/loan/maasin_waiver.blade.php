...
<body>
	<div class="a" style="text-align:center;">
		...
		<p style="margin-top:-10px" ><span class="head_others"><b>({{config('variables.coop_abbr')}})</b> TIN-{{config('variables.coop_tin')}}</span></p>
		<p style="margin-top:-10px" ><span class="head_others">{{config('variables.coop_address')}}</span></p>
		
	</div>
	@if($loan_details->memb_type > 1)
	<div class="a" style="text-align:center;">
	...
	<div class="a body_div">
		<p class="body_no_indent">
			I hereby authorize the @if($loan_details->memb_type == 3) Local Government Unit, Maasin, Municipal Accounting Office @else Barangay Treasurer of Barangay <u class="fill">{{$loan_details->brgy_lgu}}</u> @endif to deduct the amount of <u class="fill">{{$worded_amount}}</u> (<u class="fill">P{{number_format($loan_details->amtz,2)}}</u>) from my monthly salary, bonuses, cash gift, clothing or from other allowances, differentials, remuneration including terminal leave benefits effective from the date of my loan granted up to date of maturity representing payment of my loan to MKMPPC. 
		</p>
	...
	<table width="100%" class="tbl_sig" style="border-spacing: 10px 0px 10px 0px;border-collapse: separate;margin-top:-0.7cm" >
		<tr>
			<td class="text_ex p-top text-center" style="width: 33.33%;border-bottom: 1px solid;">{{$loan_details->treasurer}}</td>
			<td class="text_ex p-top text-center" style="width: 33.33%;border-bottom: 1px solid;">{{$loan_details->chairman}}</td>
			<td class="text_ex p-top text-center" style="width: 33.33%;border-bottom: 1px solid;">{{$loan_details->name}}</td>
		</tr>
		<tr class="foot_lbl">
			<td class="text_ex" style="width: 33.33%;">{{$loan_details->memb_type == 2?'Bgry. Treasurer':'Admin Officer IV'}}</td>
			<td class="text_ex" style="width: 33.33%;">{{$loan_details->memb_type == 2?'Bgry. Chairman':'Municipal Accountant'}}</td>
	...
	<div class="row">
		<table class="tbl_sig" style="width:16cm" >
			<tr>
				...
				<td style="border-bottom: 1px solid;">{{$loan_details->name}}</td>
			</tr>
			<tr>
				<td class="wspace" style="width:4.4cm"><b>AMOUNT GRANTED:</b></td>
				<td style="border-bottom: 1px solid;">{{$loan_details->principal_amount}}</td>
			</tr>
			<tr>
				<td class="wspace" style="width:4.4cm"><b>DATE GRANTED:</b></td>
				<td style="border-bottom: 1px solid;">{{$loan_details->date_released}}</td>
			</tr>
			<tr>
				<td class="wspace" style="width:4.4cm"><b>DUE GRANTED:</b></td>
				<td style="border-bottom: 1px solid;">{{$loan_details->maturity_date}}</td>
			</tr>
		</table>
	</div>
	<div class="a body_div" style="margin-top: 0mm;">
		<p class="p_body"><b>FOR VALUE RECEIVED</b>, I/We promise to pay jointly and severally to the order of <b>{{config('variables.coop_name')}}</b>, a cooperatively duly organized and existing under Philippine laws with principal office located at {{config('variables.coop_address')}} the sum of <b>PESOS:</b> <u class="fill">{{$worded_principal}}</u> (<u class="fill">P{{number_format($loan_details->principal_num,2)}}</u>) Philippine Currency with interest rate of <u class="fill">{{$worded_interest}}</u> percent ({{$loan_details->interest_rate}}%) per annum from this date hereof, until fully paid on or before the due date, according to the attached schedule of payments. The said principal and interest shall be paid on monthly installment of <b>PESOS: </b><u class="full">{{$worded_amount}}</u> (<u class="fill">P{{number_format($loan_details->amtz,2)}}</u>) Philippine Currency, for <u class="fill">{{$loan_details->terms ?? 1}}</u> months effective <u class="fill">{{$loan_details->date_released}}</u> and every month thereafter until <u class="fill">{{$loan_details->maturity_date}}</u>
		</p>
	...
	<div class="row" style="margin-top:12mm">
		<table class="tbl_sig" width="100%">
			<tr class="text-center">
				<td width="50%" style="padding-left: 1mm;padding-right: 1mm"><div style="border-bottom:1px solid;">{{$loan_details->name}}</div></td>
		...
		<table class="tbl_sig" width="100%" style="margin-top: 9mm;">
			<tr class="text-center">
				<td width="50%" style="padding-left: 1mm;padding-right: 1mm"><div style="border-bottom:1px solid;">{{$comakers->comaker_name ?? ''}}</div></td>
	...
	@include('loan.maasin_waiver_pg2')
</body>
</html>
