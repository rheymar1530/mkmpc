					...
					<div class="form-group row">
						...
						<div class="col-md-8">
							<select id="sel_loan_service" class="form-control p-0"  required <?php echo $disabled_loan_service; ?> >
								...
								@foreach($loan_service_lists as $loan_s)
									<option value="{{$loan_s->id_loan_service}}" <?php echo ($selected_loan_service == $loan_s->id_loan_service)?"selected":""; ?> >{{$loan_s->name}}</option>
								@endforeach
							</select>
						</div>
					</div>	
